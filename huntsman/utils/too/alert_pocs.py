...
################################
    ...
    def send_alert(self, citation, targets):
        ...
        if citation == 'followup':
            self.sender.send_message('POCS-SCHEDULE', {'message': 'remove', 'targets': targets})
            message = 'add'
        else:
            message = 'add'
    ...
    def get_type_of_alert(self, alert):
        ...
        Args:
            - alert (str): can be "initial" or "update".

        Returns:
            - message (str): corresponding message to input. Can be any of "add" or "followup"."""
