...
class GravityWaveEvent(object):
    ...
    def __init__(self, skymap, observer=None, galaxy_catalog='J/ApJS/199/26/table3',
        ...
        Attribs:
            ...
            - skymap (healpix map): probability map of the event downloaded from the FITS file.
            ...
            - dist_cut (float): the maximum distance to which we want to observe galaxies from catalog.
                Default is 50.0.
            - header (dictionary): header of the downloaded FITS file.
    ...
    def alert_in_time(self, tiles, time):
        ...
        t = Timer(time, self.alerter.send_alert, args=(self.header['NOTICE'], tiles))
        t.start()
    ...
    def get_prob_red_dist(self, catalog, skymap):
        ...
        Args:
            ...
            - skymap (healpix map): probability distribution for each galaxy.
        ...
        prob, distmu, distsigma, distnorm = hp.read_map(skymap, field=range(4))
    ...
    def tile_sky(self):
        ...
        dp_dV, z, r = self.get_prob_red_dist(self.catalog, self.skymap)
