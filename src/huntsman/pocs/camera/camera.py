...
class AbstractHuntsmanCamera(AbstractCamera):
    ...
    def take_recording(self, observation, headers=None, filename=None, blocking=False, **kwargs):
        ...
        if self.is_primary:
            if 'POINTING' in metadata:
                observation.pointing_images[image_id] = files_dir
            else:
                observation.exposure_list[image_id] = files_dir
    ...
    def _setup_recording(self, observation, headers, filenames, **kwargs):
        ...
        self.logger.debug(f'Setting files_dir={image_dir}')
        ...
        # The exptime header data is set as part of observation but can
        # be overridden by passed parameter so update here.
        exptime = kwargs.get('exptime', observation.duration.value)
        ...
        self.logger.debug(
            f'Observation setup: exptime={exptime!r} files_dir={image_dir!r} '
            f'image_id={image_id!r} metadata={metadata!r}')

        return exptime, image_dir, image_id, metadata
