...
async def test_monitor_rates(monitor: HuntsmanMonitor,    stream_configs):
    ...
    mem_cfg, disk_cfg = stream_configs
    mem_streams, _ = await start_streams(monitor.js, num_streams=1, mem_cfg=mem_cfg, disk_cfg=disk_cfg)
    memory_stream = mem_streams[0].config.name
    ...
    await publish_messages(monitor.js, subject, 10)
    ...
    subscription = await monitor.js.pull_subscribe(subject, durable="1")
