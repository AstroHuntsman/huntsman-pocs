...
async def list_streams(js: JetStreamContext) -> Tuple[List[str], List[str]]:
...
async def setup_streams(js: JetStreamContext) -> Tuple[List[str], List[str]]:
    ...
    if len(memory_streams) != len(disk_streams):
        print(
            f"Warning: Unequal number of memory ({len(memory_streams)}) and disk ({len(disk_streams)}) streams.")

    return memory_streams, disk_streams


async def get_memory_usage() -> int:
...
async def get_stream_stats(js: JetStreamContext, memory_streams: List[str], disk_streams: List[str]) -> Dict[str, any]:
                ...
                if time_diff > 0:
                    ...
                    for consumer in consumers:
                        if "last_delivered" in last_message_counts[stream_name]:
                            consumed += max(0, consumer["delivered"] - last_message_counts[stream_name]
    ...
    total_memory_messages = sum(stats["streams"][name]["messages"]
                                for name in memory_streams if name in stats["streams"])
    total_disk_messages = sum(stats["streams"][name]["messages"]
                              for name in disk_streams if name in stats["streams"])
    ...
    total_produced_rate = sum(stats["streams"][name]["produced_rate"] for name in list(
        memory_streams) + list(disk_streams) if name in stats["streams"])
...
async def check_and_move_messages(js: JetStreamContext, memory_streams: List[str], disk_streams: List[str]):
    ...
    try:
        ...
        print(
            f"Memory usage {memory_usage:.2f}% exceeds threshold {MEMORY_THRESHOLD}%. Moving messages to disk...")
